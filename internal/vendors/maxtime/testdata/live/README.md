...
`healthy.json` is a recorded response envelope from a lab MAXTIME controller,
captured on 2026-08-28 through the read-only `/maxtime/api/mibs/<name>`
endpoints. Each top-level key is one MIB name; each value is the controller's
JSON array response body exactly as returned (one record per MIB).
...
same lab controller on 2026-09-08. The live endpoint returns a fixed ~10 001-event
...
The fixtures are test input only. Live verification still runs against a
