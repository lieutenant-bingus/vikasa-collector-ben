...
| ADR 0010 amends ADR 0002 (dependency-pinning clause) | ADR 0002 | Was No at first probe (Status was bare `Accepted (2026-07-12)`). Fixed per the coordinator's extended authorization (fix-report appendix) — see the verified diff under "ADR 0002's status does not name ADR 0010 as an amendment," below. | Fixed (status-line correction, see fix-report appendix) |
...
| ADR 0011 amends ADR 0009 (default-grammar, single-binding consequences) | ADR 0009 | Was No at first probe. Fixed per the coordinator's extended authorization (fix-report appendix) — see the verified diff under "ADR 0009's status does not name ADR 0011 as amending it," below. | Fixed (status-line correction, see fix-report appendix) |
